...
(() => {
    ...
    const isDarkTheme = savedTheme == "dark" || (!savvedTheme && systemPreferDark);
...
// filling prompt input with random inputs 
...
promptBtn.addEventListener("click", () => {
    const prompt = examplePrompts[Math.floor(Math.random() * 10)];
    promptInput.value = prompt;
    promptInput.focus();
})

// Calcualte width / height based on chosen ratio
const getImageDimension = (aspectRatio, baseSize = 512) => {
    ...
    //Ensure dimension are multiples of 16 (Ai modol requirements)
...
const createImageCards = (selectModel, imageCount, aspectRatio, promptText) => {
    ...
    for (let i = 0; i < imageCount; i++) {
        ...
        <div class="img-card  loading" id="img-card-${i}" style="aspect-ratio: ${aspectRatio}">
                        ...
                        <img src="testImage.avif" alt="Generated Image" class="result-img">                  
                    </div>`;
    }
